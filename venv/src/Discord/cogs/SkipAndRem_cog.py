...
class SkipAndRem_cog(commands.Cog):
    ...
    async def remove(self, ctx, *args):
        ...
        if num_queue > len(music_queue):
            await ctx.send("Does not exist in queue!!!")

        elif vc is not None and document['is_Playing']:
            if num_queue <= len(music_queue):
                voice_channel = ctx.author.voice.channel
                num_members = len(voice_channel.members) - 1  # account for bot
                num_success_vote = (num_members // 2) + 1
                ...
                if num_members <= 2:
                    removed = music_queue[num_queue - 1]['title']
                    music_queue.pop(num_queue - 1)
                    ...
                    try:
                        ...
                        if Check_Count >= num_success_vote:
                            removed = music_queue[num_queue - 1]['title']
                            user_id = music_queue[num_queue - 1]['queuer']
                            music_queue.pop(num_queue - 1)
                            collection.update_one({"_id": server_ID}, {"$set": {"music_queue": music_queue}})
